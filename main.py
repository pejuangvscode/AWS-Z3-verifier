...
def _verdict(result: str) -> str:
    return "⚠️  VULNERABLE" if result == "SAT" else "✅  SAFE"
...
def _print_row(label: str, result: str, model: ModelRef | None = None) -> None:
    ...
    print(f"  {label:<38}: {result:<5} {_verdict(result)}")
    if model is not None:
        # Format the model as a compact key=value string
        assignments = ", ".join(
...
def main() -> None:
    """Entry point: parse CLI args, run all scenarios, print the summary."""
    ...
    def run(label: str, fn: Any, *args_: Any) -> None:  # noqa: ANN001
